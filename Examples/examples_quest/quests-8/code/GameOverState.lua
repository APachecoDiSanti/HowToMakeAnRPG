...
function GameOverState:Render(renderer)
    ...
    renderer:DrawRect2d(System.ScreenTopLeft(), System.ScreenBottomRight(), Vector.Create(0,0,0,1))

    if self.mWon then
        CaptionStyles["title"]:Render(renderer, "The End")
        CaptionStyles["subtitle"]:Render(renderer, "Want to find out what happens next? Write it!")
    else
