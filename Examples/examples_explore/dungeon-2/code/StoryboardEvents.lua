...
-- For operations that happen instantly but still need to return an event object
...
function FadeScreen(id, duration, start, finish)
    ...
    return function(storyboard)
        ...
        return TweenEvent:Create(
            ...
            target,
            function(target, value) target.mColor:SetW(value) end
        )
    end
end


function FadeInScreen(id, duration) return FadeScreen(id, duration, 0, 1) end
function FadeOutScreen(id, duration) return FadeScreen(id, duration, 1, 0) end
...
function NoBlock(f)
    return function(...)
        local event = f(...)
        event.IsBlocking = function() return false end
        return event
    end
end
