...
function ArenaState:Create(world, stack)
  local this = {
    ...
    mStack = stack,
    mRounds = {
      {mName = "Round 1", mLocked = false},
      {mName = "Round 2", mLocked = true},
      {mName = "Round 3", mLocked = true},
      {mName = "Round 4", mLocked = true},
      {mName = "Round 5", mLocked = true},
    }
  }
  ...
  this.mSelection = Selection:Create{
    ...
    OnSelection = function(index, item)
...
function ArenaState:HandleInput()
  if Keyboard.JustReleased(KEY_BACKSPACE) or Keyboard.JustReleased(KEY_ESCAPE) then
    self.mStack:Pop()
  end
